...
- [x] Разбить `request-pipeline.md` на модульную документацию
- [x] Создать `types/pipeline.ts` — все интерфейсы (LeagueConfig, MatchInfo, PipelineResult, AnalysisReport)
- [x] Создать `lib/config/leagues/khl.ts` — стартовый конфиг КХЛ
- [x] Проверить, что Telegram Mini App интеграция работает (layout, validate)
